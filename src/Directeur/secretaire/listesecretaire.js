...
import AjouterPatient from './ajoutersecretaire';
import Modifiersecretaire from './modifiersecretaire';
...
export default function Listesecretaire(){
  const [searchText, setSearchText] = useState('');

    const [patients, setPatients] = useState([]);
    const fetchUsers = async () => {
      const response = await fetch('/api/listeAssistante');
      const data = await response.json();
      const filteredPatients = data.filter((patient) => {
        if (!patient || !patient.nom) {
          return false;
        }
        const name = patient.nom.toLowerCase();
  ...
  const handleDelete = async () => {
    const { error } = await supabase.from('Assistant').delete().eq('idA', selectedPatientId);
    const { errorA } = await supabase.from('user').delete().eq('iduser', selectedPatientIduser);
  ...
  function handleShowPopup(patientId) {
      setSelectedPatientId(patientId.idA);
...
<span class="data-title title this " >Liste des Assisatnts</span>

</div>
<div class="data names">

   
<button class="mybtn1" onClick={handleAjouterClick}>ajouter assistant</button>
                          {showFormA ? (
...
<AjouterPatient></AjouterPatient>
              ...
              <span class="data-list">{patient.idA}</span>
              ))}
           </div>
                    <div class="data names">
                       ...
                       <span class="data-list">{patient.nom}</span>
                       ))}
                    </div>
  ...
  <span class="data-list">{patient.Adresse}</span>
  ))}
                    </div>
  ...
  <span class="data-list">{patient.email}</span>
                    ...
                    <div class="data email">
                        <span class="data-title">date recrutement</span>
                        {patients.map((patient) => (
  
  <span class="data-list">{patient.DateR}</span>
  ))}
                    </div>
      ...
      {patients.map((patient) => (
          <tr key={patient.name}>
           ...
           <div className="modal-overlay2" onClick={handleCloseModifier}>
             <div className="modal" onClick={(e) => e.stopPropagation()}> 
                  <Modifiersecretaire></Modifiersecretaire>
             </div>
           </div>
         ) : null}
                         </td>
      ...
      {patients.map((patient) => (
          <tr key={patient.date}>
                            ...
                            <td>
                                 ...
                                 <button class="bx bx-trash-alt data-list m m2" onClick={() => handleShowPopup(patient)}></button>
                                 {showPopup ? (
