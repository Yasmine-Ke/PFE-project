...
export default function Lettre(){
    ...
    useEffect(() => {
      async function fetchRdvs() {
         ...
         const { data: ordonnances, error: ordonnancesError } = await supabase.from('Lettre').select('*');
        if (ordonnancesError) {
...
<span class="data-list">dr . {medecin.name}</span>
...
<StyledDivLettre>
    <h3 id="content-to-hide">Mes Lettres d'orientation </h3>
    {rdvs.map((medecin) => (
    <li key={medecin.id}>
...
const StyledDivLettre = styled.div`
    .data1{
        margin-top:30px;
       }
       .data{
