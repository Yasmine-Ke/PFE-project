...
function WorkHours({ selectedDay  }) {
  ...
  const insertPatient1 = async (selectedHour) => {
    ...
    const { data, error } = await response.json();
    if (!error) { // vÃƒÂ©rifier si l'insertion a rÃƒÂ©ussi
      console.log('fichier inserted successfully');
      alert('Data inserted successfully');
      window.location.reload();
    } else {
      console.log('Insertion failed:', error);
      alert('Insertion failed: ' + error.message); // afficher une alerte en cas d'ÃƒÂ©chec
    }
  };
...
<label class="checkbox-label">
  <input type="checkbox" class="checkbox-input" onClick={ handleShowT}/>
  <span class="checkbox-text">voulez vous une teleconsultation enligne ? </span>
</label>
...
<>
       ...
       <button class="btn"onClick={handleSubmit2} >Confirmer la teleconsultation</button>
         </>
        ):(
          <>
<br/>
<div className="form-group">
  ...
  >
    ...
    <option value="domicile">Domicile</option>
          ...
          <button class="btn"onClick={handleSubmit} >Confirmer le rdv</button>
